...
bathymetry_carstm = function(p=NULL, DS=NULL, sppoly=NULL, redo=FALSE, map=FALSE, ...) {
  ...
  if (is.null(id)) id = paste( p$resolution, p$spatial.domain, p$overlay, p$strata_type, sep="_" )
  ...
  if (DS=="areal_units") {
    ...
    sppoly = areal_units(
      strata_type=p$strata_type,
      resolution=p$resolution,
      spatial.domain=p$spatial.domain,
      proj4string_planar_km=p$proj4string_planar_km,
      overlay= ifelse(!exists("overlay", p) || !is.finite(p$overlay) || !p$overlay, "none", p$overlay),
      constraint=ifelse(!exists("constraint", p) || !is.finite(p$constraint) || !p$constraint, "none", p$constraint),
      redo=TRUE
    )
