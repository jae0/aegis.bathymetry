  ...
  bathymetry_db = function( p=NULL, DS=NULL, varnames=NULL, redo=FALSE, ... ) {
    ...
    if (DS %in% c("baseline_prediction_locations", "baseline_prediction_locations.redo") ) {
      ...
      Z$z = aegis_lookup( data_class="bathymetry", LOCS=Z[, c("lon", "lat")], project_class="core", output_format="points" , DS="aggregated_data", variable.name="z.mean" )  # core == unmodelled
